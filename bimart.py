...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler()]  # Explicitly set StreamHandler for console logs
)
logger = logging.getLogger(__name__)
...
def trading_loop(api_key, api_secret, api_memo, token_name_bit, sellpricef, sellpricet, buypricef, buypricet, timef, timet):
    ...
    while trading_active:
        try:
            ...
            mexc_buy=float(mexc_buy)
            mexc_buyquant=float(mexc_buyquant)
            mexc_sell=float(mexc_sell)
            mexc_sellquant=float(mexc_sellquant)
            
            # Calculate the price difference
            diff1=float(mexc_sell - mexc_buy)
            diff = round(diff1,7)
...
def get_logs():
    logger.info("Fetching logs endpoint hit.")  # Debug log for route testing
